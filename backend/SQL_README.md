...
- Main schema file: `backend/database/ams-schema.sql` (Base) and `migration_alter_schema.sql` (Current)
...
The application uses SQL Server for users, roles, profiles, colleges, and applications.
...
| `Students` | Minimal table linking `Users` to an `ApplicationNumber`. |
| `StudentProfiles` | Stores detailed student personal information. |
| `StudentAcademicDetails` | Stores marks and qualifications. |
| `StudentDocuments` | Stores SharePoint URLs for uploaded files. |
| `Colleges` | Stores college profile details linked to `Users`. |
| `CollegeCourses` | Maps colleges to specific courses and branches. |
| `Applications` | Stores student application status for specific college courses. |
...
Users
  UserID
    ...
    v
Students -> StudentProfiles (1:1)
         -> StudentAcademicDetails (1:1)
