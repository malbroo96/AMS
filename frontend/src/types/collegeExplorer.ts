...
export function mapProfileToExplorerCollege(profile: CollegeProfileData, index = 0): CollegeExplorerItem {
  ...
  return {
    ...
    rating: profile.rating != null ? Number(profile.rating) : numberValue(extra.rating, 0),
    reviewCount: numberValue(profile.reviewCount ?? extra.reviewCount, 0),
    courses,
    branches,
    feesFrom: Number.isFinite(feesFrom) ? feesFrom : 0,
    studyModes: studyModes.length ? studyModes : [],
